...
from Persistent import Data, FixedSet
...
class FixedMap(FixedSet):
    ...
    def set(self, key, val):
        address = self.__get_address__(key.bytes)
        ...
        if index != None:
            self.file_object.seek(address + key.size + index)
            self.file_object.write(val.bytes)
            return True
        index = self.__find_by_bytes__(self.empty_cell, bytes)
        if index != None:
            self.file_object.seek(address + index)
            self.file_object.write(key.bytes)
            self.file_object.write(val.bytes)
            return True
        return False
