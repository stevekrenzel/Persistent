...
class Vector:
    ...
    def __init__(self, file_object, format, initial_allocation=1024, address=None):
        ...
        if self.address == None:
            ...
            # Load arrays
            self.arrays = [Array(self.file_object, self.format, address=i) for i in self.pointers if i > -1]
    ...
    def add_array(self):
        ...
        new_array = Array(self.file_object, self.format, self.initial_allocation)
...
if __name__ == "__main__":
    ...
    size   = 1
    num    = 10
    vector = Vector(db, format, initial_allocation = size)
    t = time()
    for i in range(num):
        vector[i]["age"] = 10
    print time() - t
    ...
    db.close()
    db = open(filename, "r+b")
    vector = Vector(db, format, address = vector.address)
    t = time()
    for i in range(num):
        if vector[i]["age"] != i:
            print "OH NO!"
